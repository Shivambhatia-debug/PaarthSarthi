...
export default function AdminPanel() {
  ...
  const startEditAlumni = (a: any) => {
    ...
    const expertise = safeParse(a.expertise)
    setAlumniForm({
  ...
  const startEditMentor = (m: any) => {
    ...
    const spec = safeParse(m.specialization)
    const lang = safeParse(m.languages)
    setMentorForm({
      name: m.name || "", designation: m.designation || "", company: m.company || "",
      email: m.email || "", phone: m.phone || "", bio: m.bio || "",
      specialization: spec.join(", "),
      languages: lang.join(", "),
  ...
  const startEditStartup = (s: any) => {
    ...
    const services = safeParse(s.services)
    setStartupForm({
          ...
          <div className="flex gap-1 overflow-x-auto pb-2 -mx-4 px-4 scrollbar-hide">
            {TABS.map((tab) => (
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap transition-colors shrink-0 ${activeTab === tab.id
